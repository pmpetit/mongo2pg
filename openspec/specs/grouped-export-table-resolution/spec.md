...
When multiple source collections map to the same grouped target table, export MUST produce one consolidated CSV artifact per target table without row loss caused by overwrite.
