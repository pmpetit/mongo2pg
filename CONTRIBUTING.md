...
The `src/` tree is organized into layers, each with a clear ownership boundary:

- `src/cli/` — Clap argument/subcommand definitions only (`args.rs`, `commands.rs`). No
  business logic.
- `src/commands/` — One handler module per subcommand (`infer.rs`, `export.rs`, `import.rs`,
  `to_pg.rs`, `report.rs`, `ping.rs`, `init.rs`), dispatched via `commands::run_command`.
  Handlers orchestrate a command's flow but delegate connector I/O to `db::*` and pure
  logic to `engine::*`. `commands/shared.rs` is an interim module for helpers still used
  by more than one handler.
- `src/db/` — Connector adapters: `mongo.rs` (MongoDB client construction), `pg.rs`
  (PostgreSQL client construction, SSL mode handling), `kafka.rs` (Kafka/rdkafka security
  config). Nothing here depends on `commands::*`.
- `src/engine/` — Pure, deterministic business logic: `analyzer.rs` (schema inference),
  `ddl.rs` (SQL/DDL generation), `checksum.rs` (MD5 checksum computation — the one
  exception that depends on `db::mongo`/`db::pg` client types because it reads live data),
  `stats.rs` (schema statistics), `mapping.rs` (Mongo path mapping helpers).
- Other top-level modules (`export.rs`, `groups.rs`, `report.rs`, `schema_diagram.rs`,
  `pg_table_count.rs`, `util.rs`) hold supporting logic not yet assigned to a layer.

See `openspec/changes/refactor-cli-commands-db-engine-layout/design.md` for the full
